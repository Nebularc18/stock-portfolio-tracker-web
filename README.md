...
Published container images now use readable tracking tags so GHCR history is easier to audit and roll back:

- `latest`: current default-branch image
- `main`: moving tag for the latest `main` commit
- `sha-<commit>`: immutable commit tag generated by Docker metadata
- `build-YYYYMMDD-HHMMSS-<sha>`: immutable human-friendly build tag for each non-tag publish
- `vX.Y.Z`, `X.Y.Z`, `X.Y`: release tags created when you push a Git tag such as `v1.4.0`

The publish workflow also writes a GitHub Actions summary for each run with the exact digest and every tag that was pushed for that image.

To publish a tracked release image:
...
To deploy or roll back to a specific published image, set `IMAGE_TAG` in your server `.env` file to one of those immutable tags, for example:

```env
IMAGE_TAG=build-20260330-184500-a1b2c3d
```
