    ...
    def get_dividends(self, ticker: str, years: int = 5) -> list:
        ...
        Returns:
            list: List of dividend records sorted newest first. Each record is a dict with keys:
                ...
                - payment_date (str|None): Payment date in "YYYY-MM-DD" format when available (Avanza may provide this), otherwise `None`.
                - dividend_type (str|None): Dividend type when available (e.g., ordinary, special), otherwise `None`.
        """
        ticker = ticker.upper()
