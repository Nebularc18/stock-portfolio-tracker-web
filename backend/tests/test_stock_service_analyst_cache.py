...
def test_single_and_aggregate_analyst_caches_do_not_collide(monkeypatch):
    """
    Verifies that single-source and aggregate analyst caches do not collide.
    
    Mocks file cache I/O and recommendation providers, then asserts that get_analyst_recommendations returns a list, get_all_analyst_recommendations returns a dict, the aggregate 'yfinance' entry equals the single-source result, and the aggregate 'finnhub' entry is None.
    """
...
def test_marked_empty_aggregate_cache_is_respected(monkeypatch):
    """
    Verifies that an aggregate analyst cache entry explicitly marked as empty is honored and prevents refetching.
    
    This test stubs the file cache to return a hit for the aggregate analyst cache with `has_recommendations` set to False and both providers set to `None`, then asserts the service returns {"yfinance": None, "finnhub": None} without invoking provider fetch methods.
    """
    service = StockService()
    stock_service._ANALYST_ALL_CACHE.clear()
...
def test_import_yfinance_forces_csrf_strategy(monkeypatch):
    ...
    class FakeYfData:
        def _set_cookie_strategy(self, strategy):
            """
            Record the provided cookie strategy by appending it to the surrounding test's calls list.
            
            Parameters:
                strategy (str): The cookie strategy identifier to record.
            """
    ...
    def fake_import_module(name):
        """
        Provide a test-only import hook that returns the mocked yfinance module and rejects any other module name.
        
        Parameters:
            name (str): The module name requested by the importer.
        
        Returns:
            The mocked `yfinance` module when `name` is "yfinance".
        
        Raises:
            AssertionError: If `name` is not "yfinance".
        """
        if name == "yfinance":
