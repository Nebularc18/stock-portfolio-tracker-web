...
/**
 * Determines whether a given string corresponds to one of the predefined history range keys.
 *
 * @param value - The string to test for membership in HISTORY_RANGE_OPTIONS keys
 * @returns `true` if `value` is a valid `HistoryRangeKey`, `false` otherwise.
 */
...
/**
 * Retrieve the persisted dashboard history range for the specified user or fall back to the global/ default value.
 *
 * @param userId - Optional user identifier; when provided, a per-user stored range is preferred over the global value.
 * @returns The stored `HistoryRangeKey` if present and valid, otherwise `DEFAULT_HISTORY_RANGE`.
 */
...
/**
 * Compute how many milliseconds remain until the next dashboard auto-refresh should run.
 *
 * @param now - Current time in milliseconds since the UNIX epoch; overrideable for testing.
 * @returns Milliseconds until the next scheduled auto-refresh, never less than DASHBOARD_AUTO_REFRESH_MIN_DELAY_MS and including DASHBOARD_AUTO_REFRESH_BUFFER_MS.
 */
 ...
 *
 * Accepts numeric epochs (seconds or milliseconds), ISO date-time strings (with or without an explicit timezone),
 * and date-only strings (YYYY-MM-DD). If an ISO date-time lacks a timezone it is interpreted as UTC; date-only
 * inputs are treated as midnight UTC.
 *
 * @param value - A numeric epoch, an ISO date-time (optionally without timezone), or a date-only string
 * @returns The parsed Date (in UTC). Returns an invalid Date if the input cannot be parsed
 */
...
/**
 * Formats an x-axis tick label for portfolio history charts according to the selected range and locale.
 *
 * @param dateValue - A Date, numeric timestamp, or date string representing the tick's time
 * @param range - History range key that determines label granularity
 * @param locale - BCP 47 locale used to localize the label
 * @param timezone - IANA timezone identifier used when formatting the date/time
 * @returns The localized label for the tick, or an empty string if the input cannot be parsed
 */
...
/**
 * Render the portfolio dashboard UI, including hero statistics, a performance chart with range selection, holdings table, and grouped upcoming dividends.
 *
 * Fetches and synchronizes portfolio, stock, exchange rate, and dividend data, applies currency conversion and localization, and manages auto-refresh, per-user history range persistence, background refreshes, and sortable lists.
 *
