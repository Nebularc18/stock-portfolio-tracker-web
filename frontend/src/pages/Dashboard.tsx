 ...
 * @returns The formatted currency string with two decimal places (e.g., `$1,234.56`)
 */
...
/**
 * Format a numeric percentage value as a signed string with two decimal places and a percent symbol.
 *
 * @param value - The percentage value to format (e.g., 1.23 represents 1.23%)
 * @returns A string with a leading `+` for values >= 0, two decimal places, and a trailing `%` (for example, `+1.23%` or `-0.50%`)
 */
...
/**
 * Format a `YYYY-MM-DD` date string as a short US locale date (e.g., "Aug 5").
 *
 * Interprets the input date in UTC to avoid local-timezone shifts.
 *
 * @param dateStr - A date string in `YYYY-MM-DD` format
 * @returns The date formatted with a short month and numeric day in en-US (e.g., `Aug 5`)
 */
...
/**
  * Renders the portfolio Dashboard: KPIs, 90-day performance chart, holdings table, and upcoming dividends.
  *
  * Fetches portfolio summary, stocks, exchange rates, history, and upcoming dividends, and displays loading and error states while data is being retrieved. Values are converted to the user's display currency and timestamps respect the configured timezone.
  *
